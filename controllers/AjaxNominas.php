...
class AjaxNominas{
    ...
    public $archivoexcel = array();


    public $modalidad_40;
    public $tarjeta_empresarial;
    public $gastos_medicos2;
    public $sindicato;
    ...
    public function registrarNomina(){
        $data = array(
            ...
            'archivoexcel'=>$this->archivoexcel,
            'modalidad_40'=>$this->modalidad_40,
            'tarjeta_empresarial'=>$this->tarjeta_empresarial,
            'gastos_medicos2'=>$this->gastos_medicos2,
    ...
    public function paginador($marcadores=""){
     ...
     public function paginador2(){

        $data = array( 'cliente2'=>$this->cliente,
                       'status'=>$this->liberado       
        ); 
 
         $paginacion = new Paginacion($this->registrosPorPagina);
         $paginacion->target($this->target);
         $totalRegistros = Nominas::contarRegistrosClientes($data);
         $paginacion->totalPaginas($totalRegistros);
         $paginacion->paginaActual($this->paginaActual);
         $paginacion->parametroCliente($this->cliente);
         $paginacion->parametrosAutorizacion($this->liberado);
         $mostrar = $paginacion->mostrar();
         $data = Nominas::mostrarLista($paginacion->limitRegistros(),$data);
         $registrosAsigandos = Nominas::registrosAsignados($this->liberado);
         $registrosAsigandos = $registrosAsigandos == 0 ? $totalRegistros : $registrosAsigandos;
        
         echo json_encode(array("error"=>false,"paginador"=>$mostrar,"html"=>$data,'total'=>$totalRegistros,'total2'=>$registrosAsigandos));
     }
    ...
    public function eliminarArchivo2(){
        if(!Nominas::eliminarArchivo2($this->id_nomina,$this->url))
            echo json_encode(array('error'=>true,'titulo'=>'El archivo no puede ser borrado','subtitulo'=>'¡Intentalo nuevamente!'));
        else
            echo json_encode(array('error'=>false));
    }
    ...
    public function actualizarArchivoExcel(){
        echo json_encode(array('error'=>false,'data'=>Nominas::archivosNominasExcel($this->id_nomina,$_SESSION['identificador'])));
    }

    public function cambioClientes(){
        if($this->otros === "true")
            echo json_encode(Nominas::agregarCliente($this->id_nomina,$this->target));
        else
            echo json_encode(Nominas::eliminarCliente($this->id_nomina,$this->target));
    }
...
if(isset($_POST['nominasCliente'])){
    ...
    $a->modalidad_40=$_POST['modalidad_40'];
    $a->tarjeta_empresarial=$_POST['tarjeta_empresarial'];
    $a->gastos_medicos2=$_POST['gastos_medicos2'];
    $a->sindicato=$_POST['sindicato'];
   
    if(!empty($_FILES["excelnominas"]["name"]))
        $a->archivoexcel = $_FILES["excelnominas"];
...
else if(isset($_POST['eliminarArchivo2'])){
    $a = New AjaxNominas();
    $a->id_nomina = $_POST['eliminarArchivo2'];
    $a->url = $_POST['rutaCarpeta'];
    $a->eliminarArchivo2();
}
...
else if(isset($_POST['actualizarExcel'])){
    $a = New AjaxNominas();
    $a->id_nomina = $_POST['actualizarExcel'];
    $a->actualizarArchivoExcel();
}

else if(isset($_POST['paginaActual2'])){
    $a = New AjaxNominas();
    $a->paginaActual=$_POST['paginaActual2'];
    $a->registrosPorPagina=$_POST['registrosPorPagina'];
    $a->target=$_POST['target'];
    $a->cliente=$_POST['cliente'];
    $a->liberado=$_POST['status'];
    $a->paginador2();
}

else if(isset($_POST['cambioClientes'])){
    $a = New AjaxNominas();
    $a->id_nomina = $_POST['cambioClientes'];
    $a->target = $_POST['capturista'];
    $a->otros = $_POST['tipoMovimiento'];
    $a->cambioClientes();
}

/*else if(isset($_POST['eliminarCliente'])){
    $a = New AjaxNominas();
    $a->id_nomina = $_POST['eliminarCliente'];
    $a->target = $_POST['capturista'];
    $a->eliminarCliente();
}*/