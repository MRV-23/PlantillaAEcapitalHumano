...
class Nominas{
    ...
    public static function mostrarSelect($id,$tabla,$validar = false){
       $select='';
        $respuesta = NominasModel::mostrarSelect($tabla, $validar);
    ...
    public static function mostrarLista($limit,$data){
        $respuesta = NominasModel::mostrarLista(Tablas::clientes(),$data,$limit);
        $arreglo = NominasModel::obtenerClientes(Tablas::usuario_cliente(),$data);
        $colorFila=false;
        $html='';
        $contador = MetodosDiversos::indice($limit);
        foreach($respuesta as $row => $item){
            
            if(array_search($item['id'], array_column($arreglo, 'id_cliente')) !== false)
                $checked='checked';
            else
                $checked='';
            
            $html .='<div class="row renglon-'.(boolval($colorFila=!$colorFila) ? 1 : 0).'" style="min-height:40px;margin:2px;">
                        <div class="col-sm-1 col-xs-12 columna-div columna-div-centrar color768"><b>'.$contador++.'</b></div>
                        <div class="col-sm-8 col-xs-12 columna-div"><span class="ocultar768 encabezado-min"><b>Cliente:</b></span>'.$item['nombre'].'</div>
                        <div class="col-sm-3 col-xs-12 columna-div columna-div-centrar768 text-center"><span class="ocultar768 encabezado-min"><b>Visualizar:</b></span><label class="switch"><input type="checkbox" class="seleccion-cliente" data="'.$item['id'].'" '.$checked.'><span class="slider round"></span></label></div></div>';
        }
       return $html;
    }

    public static function agregarCliente($id,$usuario){
        $respuesta = NominasModel::agregarCliente($id,$usuario,Tablas::usuario_cliente());
        if($respuesta){
            $total = self::registrosAsignados($usuario);
            $validar = $_SESSION['identificador'] == $usuario ? true : false; 
            $clientes = self::mostrarSelect(-1,Tablas::clientes(),true);
        }
        else{
            $total = 0;
            $validar = false; 
        ...
        return array('error'=>!$respuesta,'total'=> $total,'actualizar'=>$validar,'clientes'=>$clientes);
    }

    public static function eliminarCliente($id,$usuario){
        $respuesta = NominasModel::eliminarCliente($id,$usuario,Tablas::usuario_cliente());
        if($respuesta){
            $total = self::registrosAsignados($usuario);
            $total = $total > 0 ? $total : self::contarRegistrosClientes();
            $validar = $_SESSION['identificador'] == $usuario ? true : false; 
            $clientes = self::mostrarSelect(-1,Tablas::clientes(),true);
        }
        else{
    ...
    public static function registrosAsignados($usuario){
        return NominasModel::registrosAsignados($usuario,Tablas::usuario_cliente());
    }
    ...
    public static function mostrarNoministas2($ruta){
        ...
        $respuesta = NominasModel::mostrarNoministas2(Tablas::usuarios(),Tablas::sucursales(),Tablas::puestos(),$ruta);
         foreach($respuesta as $row => $item){
            $cadena .= '<div class="row renglon-'.(boolval($colorFila=!$colorFila) ? 1 : 0).'" style="margin:1px;">
                            <div class="col-sm-1 col-xs-12 columna-div columna-div-centrar color768"><b>'.$index++.'</b></div>
                            <div class="col-sm-4 col-xs-12 columna-div"><span class="ocultar768 encabezado-min"><b>Nombre:</b></span>'.$item["nombre"].'</div>
                            <div class="col-sm-3 col-xs-12 columna-div"><span class="ocultar768 encabezado-min"><b>Sucursal:</b></span>'.$item["sucursal"].'</div>
                            <div class="col-sm-4 col-xs-12 columna-div"><span class="ocultar768 encabezado-min"><b>Puesto:</b></span>'.$item["puesto"].'</div>
                        </div>';
         }  
         return $cadena;
    }
    ...
    public static function verificarJefatura2(){
        $respuesta = NominasModel::verificarJefatura(Tablas::usuarios());
        $sql = '';
		if($respuesta > 2){
            $respuesta = NominasModel::mostrarNoministasLista2(Tablas::usuarios(),Tablas::jefe());
            foreach($respuesta as $row => $item)
                $sql.='<option value="'.$item["id_usuario"].'">'.$item["nombre"].'</option>';
        }  
        return $sql;
    ...
    public static function registrarNomina($data){
        ...
        /*************************************** */
        if(!empty($data['modalidad_40'])){
            if(!preg_match('/^[0-9,.]{4,}$/',$data['modalidad_40']))
                return json_encode(array('error'=>true,'titulo'=>'Ocurrió un error','subtitulo'=>'Debes capturar el campo modalidad 40'));
            $data['modalidad_40']= str_replace(',','',$data['modalidad_40']);
        }
        else
            $data['modalidad_40']=NULL;

        if(!empty($data['tarjeta_empresarial'])){
            if(!preg_match('/^[0-9,.]{4,}$/',$data['tarjeta_empresarial']))
                return json_encode(array('error'=>true,'titulo'=>'Ocurrió un error','subtitulo'=>'Debes capturar el campo tarjeta empresarial'));
            $data['tarjeta_empresarial']= str_replace(',','',$data['tarjeta_empresarial']);
        }
        else
            $data['tarjeta_empresarial']=NULL;

        if(!empty($data['gastos_medicos2'])){
            if(!preg_match('/^[0-9,.]{4,}$/',$data['gastos_medicos2']))
                return json_encode(array('error'=>true,'titulo'=>'Ocurrió un error','subtitulo'=>'Debes capturar el campo gastos médicos SYS'));
            $data['gastos_medicos2']= str_replace(',','',$data['gastos_medicos2']);
        }
        else
            $data['gastos_medicos2']=NULL;

        if(!empty($data['sindicato'])){
            if(!preg_match('/^[0-9,.]{4,}$/',$data['sindicato']))
                return json_encode(array('error'=>true,'titulo'=>'Ocurrió un error','subtitulo'=>'Debes capturar el campo sindicato'));
            $data['sindicato']= str_replace(',','',$data['sindicato']);
        }
        else
            $data['sindicato']=NULL;
        ...
        if($respuesta){
            if($data['id_nomina'] !== NULL ){
                ...
                self::cargarexcel($data['id_nomina'],$data['archivoexcel']);
                ...
                self::cargarexcel($respuesta,$data['archivoexcel']);
    ...
    private static function cargarexcel($id,$archivo){
        if(empty($archivo))
            return;
        $ruta = "../intranet/documentos-nominas/excel-nominas/";
        if(!file_exists($ruta.$id))
            mkdir($ruta.$id, 0777, true);
        $info = new SplFileInfo($archivo['name']);
        $extension = strtolower($info->getExtension());
        if(in_array($extension ,array('xls','xlsx')) AND $archivo['size'] <= 25 * 1024 * 1024){
            $hoy = date("Y-m-d-His"); 
            $name = $id.'-'.'nominas'.'-'.$hoy.'-'.'.'.$extension;
            $src = $ruta.$id."/".$name;       
    ...
    public static function archivosMasivos($nomina,$destino,$archivos,$temporales,$tamanos){
            ...
            for($i=0;$i<$total;$i++){
                if(Configuraciones::administrador() != $_SESSION['identificador2'] AND $_SESSION['identificador'] != 191 AND $_SESSION['identificador'] != 365 AND $_SESSION['identificador'] != 375 AND $_SESSION['identificador'] != 348 AND $_SESSION['identificador'] != 253 AND $_SESSION['identificador'] != 343  AND $_SESSION['identificador'] != 295){
    ...
    public static function contarRegistros($data='',$ubicacion = "/asesores/nominas"){
        return NominasModel::contarRegistros(Tablas::nominas_liberacion(),Tablas::usuario_cliente(),$data,$ubicacion);
    }

    public static function contarRegistrosClientes($data=''){
        return NominasModel::contarRegistrosClientes(Tablas::clientes(),$data);
    }
    ...
    public static function mostrarNominas($limite="",$data="",$liberacion = "/asesores/nominas"){
        ...
        if($liberacion === '/asesores/liberacion'){
            ...
            //$respuesta = NominasModel::mostrarNominasX(Tablas::nominas_liberacion(),$limite,$data,$liberacion);
            $respuesta = NominasModel::mostrarNominas(Tablas::nominas_liberacion(),Tablas::usuario_cliente(),$limite,$data,$liberacion);
            ...
            foreach($respuesta as $row => $item){
                ...
                if($liberacion !== '/asesores/facturacion'){
                    ...
                    $temp=self::traducirTipoPago($item["tipo_pago"]);
                    $temp = $temp === 'VIÁTICOS ENTREGADOS(AJUSTES)' ? 'VIÁTICOS AJUS.': $temp;
                    $columna = $esquema.' '.$temp;
                }
                else{
    ...
    static public function verificarPermisoModificacion($nomina){
        if(intval($_SESSION['identificador2']) === 6 || $_SESSION['identificador'] == 195)
            return true;
        $usuario =  NominasModel::verificarPermisoModificacion($nomina,Tablas::nominas_liberacion());
        #region
        if(($_SESSION['identificador'] == 343 AND $usuario == 277) || ($_SESSION['identificador'] == 343 AND $usuario == 375)  )//temporal permiso para daniela en las nóminas de Irasema
            return true;
        #endregion
    ...
    static public function document2($ruta,$id,$nominista,$indice = 1){
        $peso = self::filesize_formatted($ruta);
        $nombre = basename($ruta);
        $info = new SplFileInfo($nombre);
        $extension = $info->getExtension();
        $ext = self::documentExt($extension);
        $visualizar2 = '<a class="btn btn-default" href="intranet/documentos-nominas/excel-nominas/'.$id.'/'.$nombre.'" download="'.$id.'-'.$indice.'.'.$extension.'">'.$ext.' '.'<b style="font-size:12px;">'.$id.'-'.$indice.'.'.$extension.'</b><div style="margin-bottom:-8px;margin-top:-2px;font-size:11px;margin-left:20px;">'.$peso.'</div></a>';
        $eliminar = $nominista == $_SESSION['identificador'] ? '<li><a href="#" class="eliminarAdjuntoNominas2" name="'.$nombre.'"><i class="fa fa-trash-o fa-fw"></i> Eliminar</a></li>' : '';
        return  '<div class="btn-group" style="margin-right:8px;margin-bottom:8px">
                        '.$visualizar2.'
                    <a class="btn btn-default dropdown-toggle" data-toggle="dropdown" href="#">
                        <span class="fa fa-caret-down" title="Opciones" style="padding-bottom:17px;"></span>
                    </a>
                    <ul class="dropdown-menu">
                        <li><a href="intranet/documentos-nominas/excel-nominas/'.$id.'/'.$nombre.'" download="'.$id.'-'.$indice.'.'.$extension.'"><i class="fa fa-download fa-fw"></i> Descargar</a></li>
                        '.$eliminar.'
                    </ul>
                </div>';
    }
    ...
    static public function eliminarArchivo2($archivo,$nomina){
        $directorio = "../intranet/documentos-nominas/excel-nominas/".$nomina."/".$archivo; 
        return unlink($directorio);
    }
    ...
    static public function archivosNominasRecibos($nomina){
        ...
        if($indexFile > 0 || $indexFile2 > 0){
            ...
            $files.='<div class="col-md-6"> 
                        <div style="min-height:40px;">Recibos de nómina PDF: <b><span style="font-size:20px;margin-right:10px;">'.$indexFile.'</span></b> XML: <b><span style="font-size:20px;margin-right:10px;">'.$indexFile2.'</span></b></div>
                    </div>';
            
        return array('archivos'=>$files,'total'=>$indexFile+$indexFile2);
    }

    static public function archivosNominasExcel($nomina,$nominista){
        $directorio = "../intranet/documentos-nominas/excel-nominas/".$nomina."/"; 
        $files = '';
        
        foreach (glob($directorio."*.*") as $nombre_fichero) {
            $files = self::document2($nombre_fichero,$nomina,$nominista);
        }

        if(empty($files))
            $files = '<b><span style="font-size:20px;margin-right:10px;">0</span></b>';
        return array('archivos'=>$files);
    }
    ...
    public static function mostrarDataNomina2($idNomina,$tipo){
        ...
        $files =  self::archivosNominas($idNomina,$datos["id_nominista"]);
        $filesx =self::archivosNominasRecibos($idNomina,$datos["id_nominista"]);
        $filesx2 =self::archivosNominasExcel($idNomina,$datos["id_nominista"]);
        $files2 = self::archivosFinanzas($idNomina,$datos["id_finanzas"]);
        $files3 = self::archivosTesoreria($idNomina,$datos["id_tesoreria"]);
        ...
        if($nominasTab === 'active'){
                $html.='<div style="margin-top: 2%;">
                            ...
                            <div class="row">
                                '.$filesx['archivos'].'
                                <div class="col-md-6">
                                    <span>Archivo anexo: </span><span id="labelTotalExcel">'.$filesx2['archivos'].'</span>
                                </div>   
                            </div>
                            <div class="callout callout-success" id="tipoEsquemaAjax" value="'.$tipo_esquema.'">Tipo de esquema: '.$tipo_esquema.'</div>
                            '.$nominaVinculada.'
                            <form method="POST" id="formularioNominasActualizar">
                                    ...
                                    <div class="row form-group rowColorGray">
                                        ...
                                        <div class="col-md-4">
                                            <label for="">8.-Retención ISN:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                    ...
                                    <div class="row form-group rowColorGray">
                                        <div class="col-md-8">
                                            <label for="">11.-Empresa pagadora IMSS:</label>
                                            '.$sysIcono.'
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-list-ol"></i>
                                        ...
                                        <div class="col-md-4">
                                            <label for="">12.-Total a depositarle IMSS:</label>
                                            '.$sysIcono.'
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                    ...
                                    <div class="row form-group rowColorWhite">
                                        <div class="col-md-8">
                                            <label for="">13.-Empresa pagadora asimilados:</label>
                                            '.$asimiladosIcono.'
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                    <i class="fa fa-list-ol"></i>
                                        ...
                                        <div class="col-md-4">
                                            <label for="">14.-Total a depositarle por asimilados:</label>
                                            '.$asimiladosIcono.'
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                    ...
                                    <div class="row form-group rowColorGray">
                                        <div class="col-md-4">
                                            <label for="">15.-Tipo de periodo:</label>
                                            '.$asimiladosIcono.''.$mixtoIcono.''.$sindicatoIcono.''.$sysIcono.'
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-list-ol"></i>
                                        ...
                                        <div class="col-md-4">
                                            <label for="">16.-Número de periodo:</label>
                                            '.$asimiladosIcono.''.$mixtoIcono.''.$sindicatoIcono.''.$sysIcono.'
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                    <i class="fa fa-list-ol"></i>
                                    ...
                                    <div class="row form-group rowColorWhite">
                                        <div class="col-md-4">
                                            <label for="">18.-Descuentos sueldos y salarios:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                    <i class="fa fa-dollar"></i>
                                                </div>
                                        ...
                                        <div class="col-md-4">
                                            <label for="">19.-Descuentos asesores:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                    <i class="fa fa-dollar"></i>
                                                </div>
                                        ...
                                        <div class="col-md-4">
                                            <label for="">20.-Descuentos terceros:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                    <i class="fa fa-dollar"></i>
                                                </div>
                                ...
                                <div style="border:2px dotted gray;">
                                    ...
                                    <div class="row form-group rowColorWhite">
                                        <div class="col-md-3">
                                            <label for="">21.-Ingreso:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["ingreso"].'" name="nominasIngreso" id="nominasIngreso2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["infonavit"].'" name="nominasInfonavit" id="nominasInfonavit2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["fonacot"].'" name="nominasFonacot" id="nominasFonacot2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">24.-Donativo:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["donativo"].'" name="nominasDonativo" id="nominasDonativo2">
                                            </div>                              
                                        </div>
                                    </div>
                                    ...
                                    <div class="row form-group rowColorGray">
                                        <div class="col-md-3">
                                            <label for="">25.-Pensión alimenticia:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                        ...
                                        <div class="col-md-3">
                                            <label for="">26.-Excedente de cargas:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput monetario actualizar" type="input" value="'.$datos["excedente_cargas"].'" name="nominasExcedenteCargas" id="nominasExcedenteCargas2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">27.-Carga patronal:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["cargas_patronal"].'" name="nominasCargaPatronal" id="nominasCargaPatronal2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">28.-Impuesto estatal: <i class="fa fa-check-circle text-green campoRetencionObligatorio2" style="display:'.$displayRetencion.';"></i> <label class="container_"><input type="checkbox" id="checkCalcularIsn2" class="actualizar" '.$checkedImpuestoEstatal.'><span class="checkmark_"></span></label></label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                    ...
                                    <div class="row form-group rowColorWhite">
                                        <div class="col-md-3">
                                            <label for="">29.-Comisión(monto):</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                        ...
                                        <div class="col-md-3">
                                            <label for="">30.-IMSS obrera:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["imss_obrera"].'" name="nominasImssObrera" id="nominasImssObrera2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">31.-Carga social IMSS:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["carga_social_imss"].'" name="nominasCargaSocialImss" id="nominasCargaSocialImss2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">32.-Prenómina IMSS:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["prenomina_imss"].'" name="nominasPrenominaImss" id="nominasPrenominaImss2">
                                            </div>                              
                                        </div>
                                    </div>
                                    ...
                                    <div class="row form-group rowColorGray">
                                        <div class="col-md-3">
                                            <label for="">33.-ISR/ISP(SP):</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" value="'.$datos["isr_isp"].'" type="input" name="nominasIsrIsp" id="nominasIsrIsp2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">34.-ISR art. 142:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["isr_142"].'" name="nominasIsr142" id="nominasIsr1422">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">35.-Cuota sindical:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["cuota_sindical"].'" name="nominasCuotaSindical" id="nominasCuotaSindical2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">36.-Despensa:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["despensa"].'" name="nominasDespensa" id="nominasDespensa2">
                                            </div>                              
                                        </div>
                                    ...
                                    <div class="row form-group rowColorWhite">
                                        <div class="col-md-3">
                                            <label for="">37.-Caja de ahorro:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["caja_ahorro"].'" name="nominasCajaAhorro" id="nominasCajaAhorro2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">38.-Descuento generales:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["descuento_imss"].'" name="nominasDescuentoImss" id="nominasDescuentoImss2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">39.-Apoyo sindical:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["apoyo_sindical"].'" name="nominasApoyoSindical" id="nominasApoyoSindical2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">40.-Descuentos comedor:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["descuento_comedor"].'" name="nominasDescuentoComedor" id="nominasDescuentoComedor2">
                                            </div>                              
                                        </div>
                                    </div>
                                    ...
                                    <div class="row form-group rowColorGray">
                                        <div class="col-md-3">
                                            <label for="">41.-Haberes:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["haberes"].'" name="nominasHaberes" id="nominasHaberes2">
                                            </div>                              
                                        </div>

                                        <div class="col-md-3">
                                            <label for="">42.-Subsidio (SP):</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control monetario iluminarIconoInput actualizar" type="text" value="'.$datos["excedente_subsidio"].'" name="nominasExcedenteSubsidio" id="nominasExcedenteSubsidio2">
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">43.-Prestamos empleado:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                        ...
                                        <div class="col-md-3">
                                            <label for="">44.-Prestamos ayudate:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control monetario iluminarIconoInput actualizar" type="input" value="'.$datos["prestamos_ayudate"].'" name="nominasPrestamosAyudate" id="nominasPrestamosAyudate2">
                                            </div>                              
                                        </div>
                                    </div>
                                    ...
                                    <div class="row form-group rowColorWhite">
                                        <div class="col-md-3">
                                            <label for="">45.-ajuste subsidio empleo:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["ajuste_subsidio_empleo"].'" name="ajusteSubsidioEmpleo" id="ajusteSubsidioEmpleo2">
                                            </div>                              
                                        </div>';
                   // if($_SESSION['identificador'] == 168 || $_SESSION['identificador'] == 187){
                            $html.='<div class="col-md-3">
                                        <label for="">46.-Gastos médicos:</label>
                                        <div class="input-group">
                                            <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                            </div>
                                            <input class="form-control monetario actualizar iluminarIconoInput" type="input" name="gastos_medicos2" value="'.$datos["gastos_medicos2"].'" id="gastos_medicos2">
                                        </div>                              
                                    </div>';
                    //}
                    ...
                    $html.=         '<div class="col-md-3">
                                            <label for="">47.-Otros:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay iluminarIconoInput actualizar monetario" type="input" value="'.$datos["otros"].'" name="nominasOtros" id="nominasOtros2">
                                            </div>                              
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">Comprobación SYS:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon" style="background:#00a65a;color:#fff;">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control monetario iluminarIconoInput" type="input" id="comprobacionSys2" value="'.$datos["sumatoria_sys"].'"disabled>
                                            </div>                              
                                        </div>
                                    </div>
                                ...
                                <div style="border:2px dotted gray;">
                                    ...
                                    <div class="row form-group rowColorGray">
                                        <div class="col-md-3">
                                            <label for="">48.-Asimilados a Salarios:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control textoMay monetario iluminarIconoInput actualizar" type="text" value="'.$datos["excedente_ingreso"].'" name="nominasExcedenteIngreso" id="nominasExcedenteIngreso2">
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">49.-Anticipo a Rendimientos:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control monetario iluminarIconoInput actualizar" type="text" name="nominasExcedenteTerceros" value="'.$datos["excedente_terceros"].'" id="nominasExcedenteTerceros2">
                                            </div>
                                        </div>';

                       // if($_SESSION['identificador'] == 168 || $_SESSION['identificador'] == 187){ 
                                $html.='<div class="col-md-3">
                                            <label for="">50.-Sindicato:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control monetario iluminarIconoInput actualizar" type="text" name="sindicato" value="'.$datos["sindicato"].'" id="sindicato">
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <label for="">51.-Tarjeta Empresarial:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control monetario iluminarIconoInput actualizar" type="text" name="tarjeta_empresarial" value="'.$datos["tarjeta_empresarial"].'" id="tarjeta_empresarial">
                                            </div>
                                        </div>';
                         //       }     
                      ...
                      // if($_SESSION['identificador'] == 168 || $_SESSION['identificador'] == 187){
                            $html.='<div class="row form-group rowColorGray">
                                        <div class="col-md-3">
                                            <label for="">52.-Modalidad 40:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon actualizar">
                                                    <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control monetario iluminarIconoInput actualizar" type="text" name="modalidad_40" value="'.$datos["modalidad_40"].'" id="modalidad_40">
                                            </div>
                                        </div>
                                    </div>';
                        //}

                        $html.=    '<h3 style="text-align:center;"><u>DESCUENTOS AL TRABAJADOR</u></h3>

                                    <div class="row form-group rowColorWhite">
                                        <div class="col-md-3">
                                            <label for="">53.-ISR:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                    <i class="fa fa-dollar"></i>
                                                </div>
                                    ...
                                    <div class="row form-group rowColorGray">
                                        ...
                                        <div class="col-md-3">
                                            <label for="">59.-Pensión alimenticia:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                    ...
                                    <div class="row form-group rowColorWhite">
                                        <div class="col-md-3">
                                            <label for="">61.-Recuperación:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                        ...
                                        <div class="col-md-3">
                                            <label for="">62.-Comisión socio o cliente:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                        ...
                                        <div class="col-md-3">
                                            <label for="">63.-Prenómina IMSS:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                        ...
                                        <div class="col-md-3">
                                            <label for="">64.-Prenómina GMM:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                    ...
                                    <div class="row form-group rowColorGray">
                                        <div class="col-md-3">
                                            <label for="">65.-Caja de ahorro:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                        ...
                                        <div class="col-md-3">
                                            <label for="">66.-Descuento ayudate:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control monetario iluminarIconoInput actualizar" type="text" value="'.$datos["descuento_ayudate"].'" name="descuentoAyudate" id="descuentoAyudate2">
                                            </div>
                                        </div>
                                       
                                        <div class="col-md-3">
                                            <label for="">67.-Otros:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                        ...
                                        <div class="col-md-3">
                                            <label for="">Comprobación Asimilados:</label>
                                            <div class="input-group">
                                                <div class="input-group-addon" style="background:#00a65a;color:#fff;">
                                                <i class="fa fa-dollar"></i>
                                                </div>
                                                <input class="form-control monetario iluminarIconoInput" type="input" value="'.$datos["sumatoria_asimilados"].'" id="comprobacionAsimilados2" disabled>
                                            </div>                              
                                        </div>
                                    </div>
                         ...
                         $html.='<div class="row">
                                    <div class="col-md-12">
                                        <h3>Anexa un archivo excel</h3>
                                        <div class="alert alert-secondary" role="alert" style="margin-bottom:-30px;">
                                            <div class="row">
                                                <div class="col-sm-6">
                                                    <span class="btn btn-primary btn-lg btn-file"><i class="fa fa-cloud-upload"></i> Cargar archivo <input type="file" class="actualizar" name="excelnominas" id="excelnominas2" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"></span> 
                                                </div>
            ...
            $html.='<div> 
                            ...
                            <div class="row">
                                '.$filesx['archivos'].'
                                <div class="col-md-6">
                                    <span>Archivo anexo: </span>'.$filesx2['archivos'].'
                                </div>   
                            </div>
                            <div style="border:2px dotted gray;padding-left:10px;">
                            <h3 style="text-align:left;"><u>CAPTURÓ NÓMINAS</u></h3>
            ...
            $html.='<div> 
                            ...
                            <div style="min-height:40px;">Comprobantes bancarios: <b><span style="font-size:20px;margin-right:10px;">'.$files['total'].'</span></b><span>'.$files['archivos'].'</span></div>
                            <div class="row"> '.$filesx['archivos'].'</div>
                            <div style="border:2px dotted gray;padding-left:10px;">
                            <h3 style="text-align:left;"><u>CAPTURÓ NÓMINAS</u></h3>
                            <div class="row">
                                <div class="col-md-6">
                                ...
                                <div style="border:2px dotted gray;padding-left:10px;margin-bottom:8px;">
                                    <h3 style="text-align:left;"><u>CAPTURÓ</u></h3>
                                        ...
                                        <div id="actualizarNominas_">
                                            ...
                                            <div class="row" style="margin-bottom:-10px;">
                                                <div class="col-md-6">
                                                    <span><b>14.-Total a depositar SIN TIMBRE: </b>'.number_format($datos["excedente_terceros"],2).'</span>
                                                </div>
                                            </div>
                                            ...
                                            <div class="row" style="margin-top:-10px;">
                                                <div class="col-md-6">
                                                    <span><b>15.-Tipo de periodo: </b>'.self::traducirTipoPeriodo($datos['tipo_periodo']).' </span>
                                                </div>
                                                <div class="col-md-6">
                                                    <span><b>16.-Número de periodo: </b>'.$datos['numero_periodo'].' </span>
                                                </div>
                                            </div>

                                            <div class="row">
                                                <div class="col-md-6">
                                                    <span><b>17.-Socios: </b>'.$datos['socios'].' </span>
                                                </div>
                                                <div class="col-md-6">
                                                    <span><b>18.-Descuentos sueldos y salarios: </b> <i class="fa fa-dollar"></i> '.number_format($datos['descuentos_sys'],2).' </span>
                                                </div>
                                            </div>

                                            <div class="row">
                                                <div class="col-md-6">
                                                    <span><b>19.-Descuentos asesores: </b> <i class="fa fa-dollar"></i> '.number_format($datos['descuentos_asesores'],2).' </span>
                                                </div>
                                                <div class="col-md-6">
                                                    <span><b>20.-Descuentos terceros: </b> <i class="fa fa-dollar"></i> '.number_format($datos['descuentos_terceros'],2).' </span>
                                                </div>
                                            </div>
        ...
        if(($data["documentoNombre"]) != NULL){
            ...
            if($extensionImagen == 'xlsx'){
                ...
                if($target === '/asesores/nominas')
                    return self::cargarRegistrosNominasNuevaVersion($ruta);
                   // return self::cargarRegistrosNominasNew($ruta);borrar
    ...
    public static function cargarRegistrosNominasNew($ruta){
        ...
        //$resultado.=$data;
        self::crearBitacoraLayout('NÓMINAS',$inicio,$start,$totalRegistrosCorrectos,$totalRegistrosErrores,$log,$logError);
        unlink($ruta);
        return array("data"=>$resultado,"totalCorrectos"=>$totalRegistrosCorrectos,"totalErrores"=>$totalRegistrosErrores,"totalAlertas"=>$totalRegistrosAlertas);
    }

    public static function cargarRegistrosNominasNuevaVersion($ruta){
        $inicio = date("d-m-Y H:i:s");
        $start = microtime(true);
        ...
        if($valorRaw!=="poid4655RSESRShhgtopodi89987kdjhdhcccv_ttr#$5yuuihuhuioyuioHHAFhh6rhYUU875yuuihuhuioyuioHHAFhh6rhYUU87___YETETEDEEDqfg45677%&&%%%RthnC50937DGD_7uKpoHu_NOMINAS"){
            unlink($ruta);
            return array("data"=>'Necesitas utilizar la última versión del layout de Nóminas (versión 29-06-2020), se anexo el cálculo automático de la retención del ISN',"totalCorrectos"=>0,"totalErrores"=>0,"totalAlertas"=>0);
        }
        ...
        foreach ($hojaActual->getRowIterator() as $fila){
            ...
            foreach ($fila->getCellIterator() as $celda) {
                ...
                if($fila > 4){
                    ...
                    if($columna != 'G')
                        $respuesta = self::validarNominas($columna,$fila,$valor,$nombresColumnas,$tipoValidacion,$sinFactura);
                    else{
                        $comisionValor=$valor; //brincamos el campo comisión (columna G) para evaluarlo al final
                        continue;
                    }
                    ...
                    if($respuesta['alerta']){//Ese campo se guarda en alertas pero permite que los demas campos de su fila se guarden
                        ...
                        if($columna == 'A')
                            ...
                            $descuentossy +=  str_replace("\"", "", $respuesta['valor']);
                        else if( ( $columna == 'AY' || $columna == 'AZ' || $columna == 'BA' || $columna == 'BB' || $columna == 'BC' || $columna == 'BD' || $columna == 'BG' || $columna == 'BH' || $columna == 'BI' || $columna == 'BJ' || $columna == 'BK')  AND $respuesta['valor'] != "" )
                            $descuentosasesores +=  str_replace("\"", "", $respuesta['valor']);
                        else if( ( $columna == 'BE' || $columna == 'BF'  || $columna == 'BM')  AND $respuesta['valor'] != "" )
                            $descuentosterceros +=  str_replace("\"", "", $respuesta['valor']);
                        ...
                        if($columna != 'J' AND $columna != 'K'){
                            $sqlCampos.=$separador.self::convertirEncabezadoNominas($columna);
                            $sqlValores.=$separador.$respuesta['valor'];
                        }
            ...
            if( ($fila > 4 AND !$flagObligatorio) AND !$finalizarLectura){
                ...
                $respuesta = self::validarNominas('G',$fila,$comisionValor,$nombresColumnas,$tipoValidacion,$sinFactura);
                if(!$respuesta['error']){//en caso de false, quiere decir que es necesario que el campo se capture y que ademas cumpla con el formato, por lo tanto no se realizará ese registro
                    $errores.= $respuesta['respuesta']."\r\n";
                    ++$totalRegistrosErrores;
                    continue;
                    ...
                    if($respuesta['valor'] != 1){
                        $sqlCampos.=','.self::convertirEncabezadoNominas('G');
                        $sqlValores.=','.$respuesta['valor'];
                    }  
                } 
                $data = $sqlCampos.$capturoCampo.") VALUES". $sqlValores.$capturoValor.")";
    ...
    public static function cargarRegistrosTesoreriaNew($ruta){
       ...
       // $resultado.=$data;
       self::crearBitacoraLayout('TESORERÍA',$inicio,$start,$totalRegistrosCorrectos,$totalRegistrosErrores,$log,$logError);
        unlink($ruta);
        return array("data"=>$resultado,"totalCorrectos"=>$totalRegistrosCorrectos,"totalErrores"=>$totalRegistrosErrores,"totalAlertas"=>$totalRegistrosAlertas);
    }
    ...
    public static function validarNominas($columna,$fila,$valor,$nombresColumnas,$tipoValidacion,$sinFactura){
        
        $encabezado = $nombresColumnas["$columna"];
        
        switch($columna){
            case 'A':
                return self::lista(self::traducirValorNominas($columna,$valor),$fila,$encabezado, self::$validacion2["$tipoValidacion"][0]);
            break;
            case 'B':
                return self::lista(self::traducirValorNominas($columna,$valor),$fila,$encabezado,self::$validacion2["$tipoValidacion"][1]);
            break;
            case 'C':
                return self::lista(self::traducirValorNominas($columna,$valor),$fila,$encabezado,self::$validacion2["$tipoValidacion"][2],true);
            break;
            case 'D':
                return self::lista(self::traducirValorNominas($columna,$valor),$fila,$encabezado,self::$validacion2["$tipoValidacion"][3]);
            break;
            case 'E':
                return self::lista(self::traducirValorNominas($columna,$valor),$fila,$encabezado,self::$validacion2["$tipoValidacion"][4]);
            break;
            case 'F':
                return self::lista(self::traducirValorNominas($columna,$valor),$fila,$encabezado,self::$validacion2["$tipoValidacion"][5]);
            break;
            case 'G':
                if($sinFactura == 65)
                    $validacion = false;
            ...
            break;
            case 'H':
                return self::lista(self::traducirValorNominas($columna,$valor),$fila,$encabezado,self::$validacion2["$tipoValidacion"][7]);
            break;
            case 'I':
                if($sinFactura == 65 AND $tipoValidacion != 6)
                    $validacion = false;
            ...
            break;
            case 'J':
                return self::lista(self::traducirValorNominas($columna,$valor),$fila,$encabezado,false);
            break;
            case 'K':
                return self::lista(self::traducirValorNominas($columna,$valor),$fila,$encabezado,false);
            break;
            case 'L':
                return self::lista(self::traducirValorNominas($columna,$valor),$fila,$encabezado,self::$validacion2["$tipoValidacion"][9]);
            break;
            case 'M':
                return self::numerico($valor,$fila,$encabezado,self::$validacion2["$tipoValidacion"][10]);
            break;
            case 'N':
                return self::lista(self::traducirValorNominas($columna,$valor),$fila,$encabezado,self::$validacion2["$tipoValidacion"][11]);
            break;
            case 'O':
                return self::numerico($valor,$fila,$encabezado,self::$validacion2["$tipoValidacion"][12]);
            break;
            case 'P':
                return self::lista(self::traducirValorNominas($columna,$valor),$fila,$encabezado,self::$validacion2["$tipoValidacion"][13]);
            break;
            case 'Q':
                return self::entero($valor,$fila,$encabezado,self::$validacion2["$tipoValidacion"][14]);
            break;
    ...
    public static function convertirEncabezadoNominas($indice){
        $arreglo = array(   'A'=>'esquema',
                            'B'=>'tipo_sindicato',
                            'C'=>'devengada',
                            'D'=>'id_cliente',
                            ...
                            'AR'=>'gastos_medicos2',/////////////////
                            'AS'=>'otros',
                            'AT'=>'excedente_ingreso',
                            'AU'=>'excedente_terceros',
                            'AV'=>'sindicato',/////////////////
                            'AW'=>'tarjeta_empresarial',/////////////////
                            'AX'=>'modalidad_40',/////////////////
                            'AY'=>'excedente_isr',
                            'AZ'=>'excedente_imss',
                            'BA'=>'excedente_gmm',
                            'BB'=>'excedente_infonavit',
                            'BC'=>'excedente_fonacot',
                            'BD'=>'excedente_prestamos',
                            'BE'=>'excedente_pension',
                            'BF'=>'excedente_clientes',
                            'BG'=>'excedente_recuperacion',
                            'BH'=>'excedente_comision',
                            'BI'=>'excedente_prenomina',
                            'BJ'=>'excedente_prenomina_gmm',
                            'BK'=>'excedente_caja_ahorro',
                            'BL'=>'descuento_ayudate',////////////
                            'BM'=>'excedente_otros',
                            'BN'=>'comentarios_nominas'      
                    );
            return $arreglo["$indice"];
    }
    ...
    public static function traducirValorNominas($columna,$valor){
        if($columna === "A")
            return self::traducirTipoEsquema2($valor);
        else if($columna === "B")
            return self::traducirTipoSindicato2($valor);
    ...
    public static function traducirTipoPago2($tipo){
        $tiposMovimiento = array('AGUINALDO'=>1,
                            ...
                            'OTROS'=>19,
                            'VIÁTICOS'=>20,
                            'VIÁTICOS ENTREGADOS(AJUSTES)'=>21);
    ...
    public static function tipoPago($tipo){
        $movimiento = self::completeSelect($tipo,21);
        return $html=   '<option value="1" '.$movimiento[0].'>AGUINALDO</option>
                        ...
                        <option value="20" '.$movimiento[19].'>VIÁTICOS</option>
                        <option value="21" '.$movimiento[20].'>VIÁTICOS ENTREGADOS(AJUSTE)</option>
