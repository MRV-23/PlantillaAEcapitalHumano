<?php
	class MetodosDiversos{
		...
		public static function formatearFecha($fecha,$abreviado,$mesNumero=false){
		...
		public static function validarFormatoMoneda($valor,$obligatorio = false){
			if(empty($valor) AND !$obligatorio)
				return NULL;
			if(!preg_match('/^[0-9,]{1,}[.]{1}[0-9]{2}$/',$valor))
				return true;
			return str_replace(',','',$valor);
		}
	
		public static function validarFormatoEntero($valor,$obligatorio = false){
			//$valor = intval($valor);
		...
		public static function validarFormatoTexto($valor,$obligatorio = false){
			if(empty($valor) AND !$obligatorio)
				return NULL;
