...
class Reportes{
	...
	public static function reportePersonalAsistencias($inicio,$fin){
		...
		->setCreator("Sistema de Intranet")
		->setLastModifiedBy('ASEEM') // última vez modificado por
		->setTitle('Reporte de asistencias')
		->setSubject('Reporte de asistencias')
		->setDescription('Este documento fue generado por el sistema de intranet')
		->setKeywords('Reporte de asistencias')
		->setCategory('Módulo Recursos Humanos');

		$hoja = $documento->getActiveSheet();
		$hoja->setTitle("Módulo Recursos Humanos");

		$respuesta = ReportesModel::reportePersonalAsistencias($inicio,$fin);
		
		$inicio = MetodosDiversos::formatearFecha($inicio,true);
		$fin = MetodosDiversos::formatearFecha($fin,true);

		$hoja->setCellValueByColumnAndRow(1,1,"Reporte de asistencias del $inicio al $fin");

		
		$hoja->mergeCells('A1:F1');
		$hoja->getStyle('A1:G3')->getFont()->setBold(true);
		$hoja->getStyle('A1:G3')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
		...
		$hoja->setCellValueByColumnAndRow($columna++,$fila,"NOMBRE");
		$hoja->setCellValueByColumnAndRow($columna++,$fila,"SUCURSAL");
		$hoja->setCellValueByColumnAndRow($columna++,$fila,"DEPARTAMENTO");
		$hoja->setCellValueByColumnAndRow($columna++,$fila,"PUESTO");
		$hoja->setCellValueByColumnAndRow($columna++,$fila,"FECHA DE REGISTRO");
		$hoja->setCellValueByColumnAndRow($columna++,$fila,"HORA DE REGISTRO");
		...
		$documento->getActiveSheet()->freezePane('A4');
		$documento->getActiveSheet()->setAutoFilter('A3:G3');

		
		$columna=1;
		$fila=4;
		//$hoja=self::asistenciasvalidaciones($hoja,$respuesta);
		foreach ($respuesta as $row => $item){  
			$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper($item['nombre']));
			$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper($item['sucursal']));
			$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper($item['departamento']));
			$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper($item['puesto']));
			$hoja->setCellValueByColumnAndRow($columna++,$fila,MetodosDiversos::formatearFecha($item['fecha'],false,true));
			$hoja->setCellValueByColumnAndRow($columna++,$fila,$item['hora']);
			switch ($item['estatus']){
				case 1:
					$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper("ENTRADA"));
					break;
				case 2:
					$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper("ENTRADA TARDE 9-10"));
					break;
				case 3:
					$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper("ENTRADA JUSTICI 10-2"));
					break;
				case 4:
					$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper("HORAS  DE COMIDA 2-4"));
					break;
				case 5:
					$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper(" SALIDA CON PERMISO 4-7"));
					break;
				case 6:
					$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper("SALIDA 7 O +"));
					break;
				default:
					$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper("ERROR EN HORA"));
					break;
			}
			$columna=1;
            $fila++;
	...
	public static function asistenciasvalidaciones($hoja,$array){
		$columna=1;
		$fila=4;
		foreach ($array as $row => $item){  
			...
			$hoja->setCellValueByColumnAndRow($columna++,$fila,MetodosDiversos::formatearFecha($item['fecha'],false,true));
			$hoja->setCellValueByColumnAndRow($columna++,$fila,$item['hora']);
			/*if ($item['estatus'] === 2 || $item['estatus'] === 3 || $item['estatus'] === 5 ){
				$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper("entrada_destiepo"));
				//$hoja->setCellValueByColumnAndRow($columna++,$fila,$item['hora']);
			}*/
			switch ($item['estatus']){
				case 1:
				...
				case 4:
					...
					break;
				case 5:
					$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper(self::busquedaPermisos()));
					//$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper(" SALIDA CON PERMISO 4-7"));
					break;
				case 6:
					$hoja->setCellValueByColumnAndRow($columna++,$fila,strtoupper("SALIDA 7 O +"));
