	...
	public static function actualizarFormularioSolicitudModels($datos,$tabla){
		...
		if($datos["autorizar"]){
			if($datos['tipoDePermiso']==2){//ejecutar el procedimiento sólo si se trata de vacaciones
				...
				if(  date($datos['fechaSolicitada']) <= date('2020-05-01')  &&  date($datos['fechaFinalizacion']) >= date('2020-05-01') )
					$totalDias -= 1;
				if(  date($datos['fechaSolicitada']) <= date('2020-09-16')  &&  date($datos['fechaFinalizacion']) >= date('2020-09-16') )
					$totalDias -= 1;
				if(  date($datos['fechaSolicitada']) <= date('2020-11-16')  &&  date($datos['fechaFinalizacion']) >= date('2020-11-16') )
					$totalDias -= 1;
				if(  date($datos['fechaSolicitada']) <= date('2020-12-25')  &&  date($datos['fechaFinalizacion']) >= date('2020-12-25') )
					$totalDias -= 1;
