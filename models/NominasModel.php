...
class NominasModel{
    ...
    public static function mostrarSelect($tabla,$validar=false){
        $clientes='';
        if($validar){
            $clientes = self::seleccionarClientes(Tablas::usuario_cliente());
            $clientes = $clientes == '' ? '' : " WHERE id IN ($clientes)";
        }
        $stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla $clientes ORDER BY nombre");
        $stmt -> execute();
        return $stmt -> fetchAll();
        $stmt -> close(); 
    }

    public static function mostrarLista($tabla,$data,$limit){
        $where=self::where2($data,$tabla);
        $stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla $where ORDER BY nombre $limit");
        $stmt -> execute();
        return $stmt -> fetchAll();
        $stmt -> close(); 
    }

    public static function agregarCliente($id,$usuario,$tabla){
        $stmt = Conexion::conectar()->prepare("INSERT INTO $tabla(id_usuario,id_cliente) VALUES(:usuario,:cliente)");
        $stmt->bindParam(':usuario',$usuario,PDO::PARAM_INT);
        $stmt->bindParam(':cliente',$id,PDO::PARAM_INT);
        return $stmt->execute();
        $stmt->close(); 
    }

    public static function eliminarCliente($id,$usuario,$tabla){
        $stmt = Conexion::conectar()->prepare("DELETE FROM $tabla WHERE id_usuario = :usuario AND id_cliente = :cliente");
        $stmt->bindParam(':usuario',$usuario,PDO::PARAM_INT);
        $stmt->bindParam(':cliente',$id,PDO::PARAM_INT);
        return $stmt->execute();
        $stmt->close(); 
    ...
    public static function registrosAsignados($usuario,$tabla){
    ...
    public static function obtenerClientes($tabla,$data){
        $stmt = Conexion::conectar()->prepare("SELECT id_cliente FROM $tabla WHERE id_usuario = :usuario");
        $stmt->bindParam(':usuario',$data['status'],PDO::PARAM_INT);
        $stmt->execute();
        return $stmt->fetchAll();
        $stmt->close(); 
    }
    ...
    public static function mostrarNoministas2($tabla,$tabla2,$tabla3,$ruta){
        ...
        else if($ruta === "/asesores/costos")
            $grupo = GrupoCostos::data();
        else if($ruta === "/asesores/conciliacion")
            $grupo = GrupoConciliacion::data();
        else
    ...
    public static function mostrarNoministasLista2($tabla,$tabla2){
        $stmt = Conexion::conectar()->prepare("SELECT id_usuario,CONCAT(nombre,' ',paterno,' ',materno) AS nombre FROM $tabla WHERE id_usuario IN (SELECT id_empleado FROM $tabla2 WHERE id_jefe = :jefe) AND situacion = 1 ORDER BY nombre,materno,paterno"); //AND situacion = 1
        $stmt->bindParam(':jefe',$_SESSION['identificador'],PDO::PARAM_INT);
        $stmt -> execute();
        return $stmt -> fetchAll();
        $stmt -> close(); 
    ...
    public static function registrarNomina2($data,$tabla){
		...
		$stmt = $conexion->prepare("INSERT INTO $tabla 
            ...
            $stmt->bindParam(':modalidad_40',$data['modalidad_40'],PDO::PARAM_STR);
            $stmt->bindParam(':tarjeta_empresarial',$data['tarjeta_empresarial'],PDO::PARAM_STR);
            $stmt->bindParam(':gastos_medicos2',$data['gastos_medicos2'],PDO::PARAM_STR);
            $stmt->bindParam(':sindicato',$data['sindicato'],PDO::PARAM_STR);
    ...
    public static function actualizarNomina2($data,$tabla){
        ...
        modalidad_40=:modalidad_40,
        tarjeta_empresarial=:tarjeta_empresarial,
        gastos_medicos2=:gastos_medicos2,
        sindicato=:sindicato
		WHERE id = :id");
    ...
    public static function contarRegistros($tabla,$tabla2,$data,$ubicacion){
        $where=self::where($data,$tabla,$ubicacion);
            ...
            if(!empty($clientes)){
                foreach( $clientes as $row => $item)
                    $clientes2 .= ','.$item['id_cliente'];
                $clientes2 = substr($clientes2 ,1);
                $clientes2 = " AND id_cliente IN ($clientes2)";
            }
            else
                $clientes2='';*/
        
        $stmt = Conexion::conectar()->prepare("SELECT COUNT(id) FROM $tabla $where $clientes");
        $stmt -> execute();
        return $stmt -> fetch()[0];
        $stmt -> close(); 
    }

    public static function contarRegistrosClientes($tabla,$data){
        $where=self::where2($data,$tabla);
    ...
    /*public static function mostrarNominasX($tabla,$limite,$data,$ubicacion){
        $where=self::where($data,$tabla,$ubicacion);
    ...
    public static function mostrarNominas($tabla,$tabla2,$limite,$data,$ubicacion){
        $where=self::where($data,$tabla,$ubicacion);
        $clientes = self::seleccionarClientes($tabla2);
        $clientes = $clientes == '' ? '' : " AND id_cliente IN ($clientes)";
        $stmt = Conexion::conectar()->prepare("SELECT id,id_nominista,id_finanzas,id_tesoreria,tipo_pago,captura_nominista,esquema,observaciones,tesoreria_estatus,liberacion_nominas,id_cliente,id_facturacion,estatus_factura FROM $tabla $where $clientes ORDER BY id desc $limite");
        $stmt -> execute();
        return $stmt -> fetchAll();
        $stmt -> close(); 
    }

    public static function seleccionarClientes($tabla){
        $clientes2='';
        $stmt = Conexion::conectar()->prepare("SELECT id_cliente FROM $tabla WHERE id_usuario = :usuario");
    ...
    public static function where2($data,$tabla){
        $where=' WHERE 1';
        if(!empty($data['cliente2']) ){
            $cadena = $data['cliente2'];
            $where .= " AND $tabla.nombre LIKE '%$cadena%' COLLATE utf8_general_ci";
        }  
       /* if(!empty($data['status']) )
            $where .= " AND $tabla.id = ".intval($data['status']); */
        return $where;
    }
...
//CONCAT(nombre,' ',paterno,' ',materno) AS
    ...
    public static function mostrarDataNomina($idNomina,$tabla){
        $stmt = Conexion::conectar()->prepare("SELECT *,
                                               SUM( IF(ingreso IS NULL,0,ingreso) +
                                                    IF(infonavit IS NULL,0,infonavit) +
                                                    IF(fonacot IS NULL,0,fonacot) +
                                                    IF(donativo IS NULL,0,donativo) +
                                                    IF(pension IS NULL,0,pension) +
                                                    IF(excedente_cargas IS NULL,0,excedente_cargas) +
                                                    IF(cargas_patronal IS NULL,0,cargas_patronal) +
                                                    IF(isn IS NULL,0,isn) +
                                                    IF(comision_monto IS NULL,0,comision_monto) +
                                                    IF(imss_obrera IS NULL,0,imss_obrera) +
                                                    IF(carga_social_imss IS NULL,0,carga_social_imss) +
                                                    IF(prenomina_imss IS NULL,0,prenomina_imss) +
                                                    IF(isr_isp IS NULL,0,isr_isp) +
                                                    IF(isr_142 IS NULL,0,isr_142) +
                                                    IF(cuota_sindical IS NULL,0,cuota_sindical) +
                                                    IF(despensa IS NULL,0,despensa) +
                                                    IF(caja_ahorro IS NULL,0,caja_ahorro) +
                                                    IF(descuento_imss IS NULL,0,descuento_imss) +
                                                    IF(apoyo_sindical IS NULL,0,apoyo_sindical) +
                                                    IF(descuento_comedor IS NULL,0,descuento_comedor) +
                                                    IF(haberes IS NULL,0,haberes) +
                                                    IF(excedente_subsidio IS NULL,0,excedente_subsidio) +
                                                    IF(prestamos_empleados IS NULL,0,prestamos_empleados) +
                                                    IF(prestamos_ayudate IS NULL,0,prestamos_ayudate) +
                                                    IF(ajuste_subsidio_empleo IS NULL,0,ajuste_subsidio_empleo) +
                                                    IF(otros IS NULL,0,otros) + 
                                                    IF(gastos_medicos2 IS NULL,0,gastos_medicos2) 
    	                                        )AS sumatoria_sys,
                                               SUM( IF(excedente_ingreso IS NULL,0,excedente_ingreso) +
                                                    IF(excedente_terceros IS NULL,0,excedente_terceros) +
                                                    IF(excedente_isr IS NULL,0,excedente_isr) +
                                                    IF(excedente_imss IS NULL,0,excedente_imss) +
                                                    IF(excedente_gmm IS NULL,0,excedente_gmm) +
                                                    IF(excedente_infonavit IS NULL,0,excedente_infonavit) +
                                                    IF(excedente_fonacot IS NULL,0,excedente_fonacot) +
                                                    IF(excedente_prestamos IS NULL,0,excedente_prestamos) +
                                                    IF(excedente_pension IS NULL,0,excedente_pension) +
                                                    IF(excedente_clientes IS NULL,0,excedente_clientes) +
                                                    IF(excedente_recuperacion IS NULL,0,excedente_recuperacion) +
                                                    IF(excedente_comision IS NULL,0,excedente_comision) +
                                                    IF(excedente_prenomina IS NULL,0,excedente_prenomina) +
                                                    IF(excedente_prenomina_gmm IS NULL,0,excedente_prenomina_gmm) +
                                                    IF(excedente_caja_ahorro IS NULL,0,excedente_caja_ahorro) +
                                                    IF(descuento_ayudate IS NULL,0,descuento_ayudate) +
                                                    IF(excedente_otros IS NULL,0,excedente_otros) +

                                                    IF(sindicato IS NULL,0,sindicato) +
                                                    IF(tarjeta_empresarial IS NULL,0,tarjeta_empresarial) +
                                                    IF(modalidad_40 IS NULL,0,modalidad_40)

                                                )AS sumatoria_asimilados 
